...
namespace Simulacrum.Hext
{
	public sealed class Mouse : MonoBehaviour
	{
		...
		public IEnumerator Start()
		{
			...
			Camera mainCamera = console.mainCamera;
			screenXMin = mainCamera.WorldToViewportPoint(console.foreground.GetComponent<MeshRenderer>().bounds.min).x;
			screenXMax = mainCamera.WorldToViewportPoint(console.foreground.GetComponent<MeshRenderer>().bounds.max).x;
			screenYMin = mainCamera.WorldToViewportPoint(console.foreground.GetComponent<MeshRenderer>().bounds.min).y;
			screenYMax = mainCamera.WorldToViewportPoint(console.foreground.GetComponent<MeshRenderer>().bounds.max).y;
		...
		public void Update()
		{
			...
			Vector2 cellPosition = new Vector2(
				Mathf.FloorToInt(((mousePosition.x - screenXMin) / (screenXMax - screenXMin)) * console.displayWidth),
				Mathf.FloorToInt(((mousePosition.y - screenYMin) / (screenYMax - screenYMin)) * console.displayHeight));

			// clamp cell position within display
			cellPosition = new Vector2(
				Mathf.Clamp(cellPosition.x, 0f, console.displayWidth - 1),
				Mathf.Clamp(cellPosition.y, 0f, console.displayHeight - 1));
			...
			if ( currentCell != null )
			{
				...
				Color clearColor = console.GetBackgroundColorForCell(
				...
				currentCell.SetContent(
					"",
					clearColor,
					fadeToClear ? console.clearColor : cursorColor,
					cursorFadeTime,
					cursorColor,
			...
			// new current cell
			currentCell = console.GetCell(reservedLayer, cellPosition.x, cellPosition.y);

			// get background color for current cell
			Color currentCellBackgroundColor = console.GetBackgroundColorForCell(
				(int) currentCell.position.x,
				(int) currentCell.position.y,
				reservedLayer);
			...
			if ( !dragging )
			{
				for ( int i = console.GetNumLayers() - 1; i >= 0; i-- )
				{
					Cell cellHover = console.GetCell(i, currentCell.position.x, currentCell.position.y);
